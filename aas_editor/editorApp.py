...
class EditorApp(QMainWindow, design.Ui_MainWindow):
    ...
    def initActions(self):
        ...
        self.actionDefault = QAction("Standard", self,
                                     ...
                                     enabled=True)
        self.actionOpenPack = QAction("Open package", self,
    ...
    def openPack(self):
        packFile = QFileDialog.getOpenFileName(self, "Open package",
        ...
        if file:
            try:
                ...
                if Path(file) in self.packTreeViewModel.openedFiles():
                    QMessageBox.critical(self, "Error", f"Package {file} is already opened")
                else:
                    self.packTreeViewModel.iterItems().addItem(pack)
